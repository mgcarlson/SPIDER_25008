  ...
  spider_interfaces__msg__LocCommands__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    spider_interfaces__msg__LocCommands * data =
      (spider_interfaces__msg__LocCommands *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!spider_interfaces__msg__LocCommands__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          spider_interfaces__msg__LocCommands__fini(&output->data[i]);
        }
        return false;
      }
    }
