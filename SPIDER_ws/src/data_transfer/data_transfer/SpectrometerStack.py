...
class SpectrometerStack:
    ...
    def data_collection(self):
        ...
        for sensor in range(len(self.spec)):
            B.append(self.spec[sensor].channel_480nm)
            G.append(self.spec[sensor].channel_555nm)
            R.append(self.spec[sensor].channel_680nm)
            N.append(self.spec[sensor].channel_nir)
        r = sum(R)/float(len(R))
        g = sum(G)/float(len(G))
        b = sum(B)/float(len(B))
        n = sum(N)/float(len(N))
...
if __name__ == "__main__":
    specStack = SpectrometerStack(False)
    specStack.collect_and_write(float(3))
