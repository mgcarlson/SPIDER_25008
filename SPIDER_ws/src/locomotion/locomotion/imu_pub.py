...
class SpiderIMUPublisher(Node):
    ...
    def timer_callback(self):
        ...
        tempQuatTuple = self.sensor.checkEuler()
        tempQuaternion = Quaternion()
        #tempQuaternion.w = cal_w
        tempQuaternion.w = 0.0
        tempQuaternion.x = float(tempQuatTuple[2])
        tempQuaternion.y = float(tempQuatTuple[1])
        #tempQuaternion.z = cal_z
        tempQuaternion.z = float(tempQuatTuple[0])
        ...
        self.logImuData(tempQuatTuple[0], tempAngTuple, tempAccTuple, currTime.seconds_nanoseconds())
        self.i += 1
